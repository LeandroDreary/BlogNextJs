...
export const getServerSideProps: GetServerSideProps = async ({ req, res }) => {
  await DbConnect();
  let { categories, warnings } = { categories: null, warnings: [] };
  var searchParams = new URLSearchParams(
    (await getRawBody(req)).toString("utf-8")
  );
  const { name, email, message } = {
    name: searchParams.get("usernnameame"),
...
const Index = ({ info, categories, warnings }) => {
  return (
    <>
      <Layout
        head={<title>Contato - {info?.websiteName}</title>}
        info={info}
        ...
        <div className="container mx-auto">
          <div className="grid grid-cols-5 my-6 text-center md:text-left">
            <div className={"col-span-5 px-4"}>
              ...
              {warnings.map((warning) => {
                if (warning?.input === "" && !warning?.success)
                  return (
                    <p
                      key={warning.message + warning.success}
                      className="w-full text-center border border-red-400 py-3 bg-red-50 text-red-400 text-xs italic font-bold my-2"
                    >
                      {warning.message}
                    </p>
                  );
                if (warning?.input === "" && warning?.success)
                  return (
                    <p
                      key={warning.message + warning.success}
                      className="w-full text-center border border-green-400 py-3 bg-green-50 text-green-400 text-xs italic font-bold my-2"
                    >
                      {warning.message}
                    </p>
                  );
            ...
            <div className="col-span-5 md:col-span-2 py-4 px-2 sm:px-8">
              <p className="my-3 text-xl text-gray-800">Ei, vamos conversar!</p>
              <p className="my-3 text-lg text-gray-600">
                Não hesite em falar conosco por meio das informações de contato
              ...
              <div className="col-span-5 py-4 md:px-4 md:col-span-2">
                <a
                  href=""
                  className="text-xl flex items-center my-5"
                  target="_blank"
                ...
                >
                  ...
                  <span className="text-gray-700 hover:underline"></span>
                </a>
                <a
                  href=""
                  className="text-xl flex items-center my-5"
            ...
            <div className="col-span-5 md:col-span-3 md:pt-4 pb-8 md:px-6">
              <form
                method="POST"
                className="mx-auto md:max-w-md bg-white px-6 border shadow-lg"
              >
                <div className="my-6">
                  <label
                    htmlFor="websiteName"
                  ...
                  <br />
                  <input
                    className="shadow appearance-none border w-full rounded py-1 px-3 text-gray-700"
                    placeholder="Nome"
                    type="text"
                    name="name"
                  />
                ...
                <div className="mb-6 mt-3">
                  <input
                    type="submit"
                    value="enviar"
                    className={`bg-${info?.colors?.background?.color} hover:bg-${info?.colors?.background?.shadow} px-4 py-2 text-${info?.colors?.text?.shadow} hover:text-${info?.colors?.text?.color} font-semibold`}
                  />
                </div>
              </form>
            </div>
