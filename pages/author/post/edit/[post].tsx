...
function Blog({ info, user, link, authors, categories }) {
    ...
    useEffect(() => {
        if (link) {
            ...
            Api.get(`/api/post`, { params, withCredentials: true }).then(response => setPost(response.data?.result))
        }
    }, [link])

    return (
        <>
            <LayoutAdmin head={<title>Editar post - {post?.title}</title>} info={info} user={user}>
                <div className="container mx-auto">
                    ...
                    <div>
                        {
                            post?._id ?
                                <Post requestAs={"author"} onSubmit={() => { Router.push('/author/post') }} info={info} Post={post} authors={authors} categories={categories} />
