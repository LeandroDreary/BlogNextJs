...
class Login extends React.Component<MyProps, MyState> {
    ...
    componentDidMount() {
        ...
        this.setState({ ...this.state, ...{ user: JSON.parse(localStorage.getItem("user")), info: JSON.parse(localStorage.getItem("info")) } })
    }
            ...
            <nav
                ...
                <div className={`flex justify-between lg:w-auto w-full lg:border-b-0 pl-6 pr-2 border-solid border-b-2 border-${this.props?.info?.colors?.text?.color || "white"} pb-5 lg:pb-0`}>
                    ...
                    <div className="block lg:hidden">
                        <button
                            id="nav"
                            className={`flex items-center px-3 py-2 border-2 rounded text-${this.props?.info?.colors?.text?.color || "white"} hover:text-${this.props?.info?.colors?.text?.color || "white"} border-${this.props?.info?.colors?.text?.color || "gray-white"} hover:text-${this.props?.info?.colors?.text?.shadow || "gray-100"} hover:text-${this.props?.info?.colors?.text?.shadow || "gray-100"} hover:border-${this.props?.info?.colors?.text?.shadow || "gray-100"}`}>
                ...
                <div className="menu w-full flex-grow lg:flex lg:items-center lg:w-auto lg:px-3 px-8">
                    ...
                    <div className="flex">
                        <button
                            className={`block text-md px-4 py-2 rounded text-${this.props?.info?.colors?.text?.color || "white"} ml-2 hover:text-${this.props?.info?.colors?.text?.color || "gray-100"} font-bold mt-4 lg:mt-0`}>{this.state.user.username}</button>
                        <button className={`block text-md px-4 py-2 rounded text-${this.props?.info?.colors?.text?.color || "white"} ml-2 hover:text-${this.props?.info?.colors?.text?.color || "gray-100"} font-bold mt-4 lg:mt-0`}>Logout</button>
                    </div>
                </div>
