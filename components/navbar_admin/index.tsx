    ...
    componentDidMount() {
        if (this.props?.user) {
            ...
            if (localStorage.getItem("user")) {
                if (JSON.parse(localStorage.getItem("user")) !== this.state.user)
                    this.setState({ ...this.state, user: JSON.parse(localStorage.getItem("user")) })
            }
        }
        if (this.props?.info) {
                ...
                <div className="menu w-full flex-grow lg:flex lg:items-center lg:w-auto lg:px-3 px-8">
                    <div className={`text-md font-bold text-${this.props?.info?.colors?.text?.color || this.state?.info?.colors?.text?.color || "white"} lg:flex-grow`}>
                        <Link href='/admin/post'>
                            <a>
                                <button
                                    className={`block mt-4 lg:inline-block lg:mt-0 hover:text-${this.props?.info?.colors?.text?.shadow || this.state?.info?.colors?.text?.shadow || "gray-100"} font-semibold px-4 py-2 rounded mr-2`}>
                                    Posts
                                </button>
                            </a>
                        </Link>
